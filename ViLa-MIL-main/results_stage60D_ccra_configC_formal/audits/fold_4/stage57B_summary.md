...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.3834，median=0.3229，ratio>0.5 占比=0.2435。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0048，csg_only acc=0.5129533678756477。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage60D_ccra_configC_formal/audits/fold_4 \
  --split test \
  --fold 4 \
