...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.2424，median=0.2572，ratio>0.5 占比=0.0052。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0005，csg_only acc=0.8144329896907216。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage60D_ccra_configC_formal/audits/fold_1 \
  --split test \
  --fold 1 \
