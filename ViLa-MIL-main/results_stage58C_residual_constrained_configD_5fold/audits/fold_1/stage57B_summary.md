...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.2684，median=0.2416，ratio>0.5 占比=0.0722。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0004，csg_only acc=0.7319587628865979。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58C_residual_constrained_configD_5fold/audits/fold_1 \
  --split test \
  --fold 1 \
