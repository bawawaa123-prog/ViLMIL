...
def process_slide_20x(slide_name, slide_folder, root_folder, save_folder):
    """处理单个WSI文件，裁剪20x patches"""
    ...
    # 裁剪patches (patch_level=1对应20x)
    for i, coord in enumerate(coords):
        x, y = coord
        patch = wsi.wsi.read_region((x, y), 1, (256, 256)).convert('RGB')
...
def main():
    ...
    root_folder = 'D:/FenLei/ViLa-MIL-main/patches_coords_xin_20x'
    save_folder = 'Z:/ljh/data/results/new/patches_20x'
    ...
    for _, row in df.iterrows():
        ...
        process_slide_20x(slide_name, slide_subfolder, root_folder, save_folder)
