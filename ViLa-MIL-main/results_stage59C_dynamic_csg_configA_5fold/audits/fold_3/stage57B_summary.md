...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.3741，median=0.3364，ratio>0.5 占比=0.1969。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0015，csg_only acc=0.7305699481865285。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage59C_dynamic_csg_configA_5fold/audits/fold_3 \
  --split test \
  --fold 3 \
