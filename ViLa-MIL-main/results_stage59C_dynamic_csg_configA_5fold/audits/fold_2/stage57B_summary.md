...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.4043，median=0.3709，ratio>0.5 占比=0.2938。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0020，csg_only acc=0.520618556701031。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage59C_dynamic_csg_configA_5fold/audits/fold_2 \
  --split test \
  --fold 2 \
