...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.4065，median=0.3630，ratio>0.5 占比=0.2320。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0004，csg_only acc=0.6804123711340206。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage59C_dynamic_csg_configA_5fold/audits/fold_1 \
  --split test \
  --fold 1 \
