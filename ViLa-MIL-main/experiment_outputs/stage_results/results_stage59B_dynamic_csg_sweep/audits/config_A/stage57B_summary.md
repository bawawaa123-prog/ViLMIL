...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.3146，median=0.2971，ratio>0.5 占比=0.1289。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0026，csg_only acc=0.8144329896907216。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage59B_dynamic_csg_sweep/audits/config_A \
  --split test \
  --fold 0 \
  --run_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage59B_dynamic_csg_sweep/rce_v2_rcD_dynCSG_A_fold0_s1
```
