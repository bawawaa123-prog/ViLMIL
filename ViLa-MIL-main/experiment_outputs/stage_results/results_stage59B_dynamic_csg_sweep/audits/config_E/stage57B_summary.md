...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.4843，median=0.3951，ratio>0.5 占比=0.3041。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0007，csg_only acc=0.5463917525773195。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage59B_dynamic_csg_sweep/audits/config_E \
  --split test \
  --fold 0 \
  --run_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage59B_dynamic_csg_sweep/rce_v2_rcD_dynCSG_E_fold0_s1
```
