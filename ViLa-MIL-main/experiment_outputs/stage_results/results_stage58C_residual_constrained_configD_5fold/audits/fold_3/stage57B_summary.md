...
- full / concept_only / visual_only / full_without_visual 哪个分支表现最好：`full`（按 `balanced_acc` 选取）。
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.3395，median=0.3012，ratio>0.5 占比=0.2228。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0009，csg_only acc=0.45077720207253885。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58C_residual_constrained_configD_5fold/audits/fold_3 \
  --split test \
  --fold 3 \
  --run_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58C_residual_constrained_configD_5fold/rce_v2_rcD_l003_t050_aux020_5fold_e20_s1
```
