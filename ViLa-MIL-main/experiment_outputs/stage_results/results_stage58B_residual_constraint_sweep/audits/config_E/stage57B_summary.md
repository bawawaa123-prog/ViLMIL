...
- full / concept_only / visual_only / full_without_visual 哪个分支表现最好：`concept_only`（按 `balanced_acc` 选取）。
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.4574，median=0.3826，ratio>0.5 占比=0.2938。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0007，csg_only acc=0.4587628865979381。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58B_residual_constraint_sweep/audits/config_E \
  --split test \
  --fold 0 \
  --run_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58B_residual_constraint_sweep/rce_v2_rc_config_E_fold0_s1
```
