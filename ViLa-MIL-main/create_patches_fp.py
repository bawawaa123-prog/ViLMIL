...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, only_mask_save_dir, stitch_save_dir,
	...
	for slide in slides:
		# 直接在 source 目录下查找 slide 文件 (不再使用 benign/non_benign 子目录)
		file_path = os.path.join(source, slide + '.svs')
	...
	for i in range(total):
		...
		# Initialize WSI from the source directory (no subdirectory lookup)
		full_path = os.path.join(source, slide + '.svs')
