...
1. **不得擅自压缩上下文**。在对话过程中，不能自动压缩上下文，如实在需要压缩上下文，请询问用户是否同意。
2. **小步迭代**。每次只完成用户明确要求的当前 Step，不要提前实现后续 Step。
3. **不要默认查看所有文件**。先根据本文件的目录地图判断任务可能涉及哪些文件，再按需打开相关文件。
4. **保护已有 baseline**。不要破坏以下已有路径：
   - `ViLa_MIL`
   - `ViLa_MIL_BiomedCLIP`
   - PEPS / SAP-PEPS / SAF 实验路径
   - 已有 `trained_models/`、`eval_results/`、历史结果目录
5. 优先**新增模型文件/脚本文件**，不要直接重写已经稳定的模型文件。
6. 保持公共接口兼容，尤其是：
...
7. 代码应结构清晰、命名明确、可读性好；涉及 tensor 操作时应写必要的 shape 注释。
8. 新脚本避免硬编码绝对路径。应使用环境变量默认值和基于脚本位置推导的 `ROOT_DIR`。
9. 新 shell 脚本优先使用 **bash 数组**，避免长反斜杠 `\` 续行。
10. 除非用户明确要求，否则只运行快速检查，不直接运行耗时任务。
11. 每次完成大的代码改动 Step 后，都要按照第 3 节要求，向 `docs/CODEX_HANDOFF.md` 追加简洁交接记录。
12. 如果要进行长时间的训练或评估，请先在 `docs/CODEX_HANDOFF.md` 中记录，并生成训练或评估脚本，提供给用户可直接执行命令，让用户自行执行并且要求用户执行完后告知，然后进行分析。
