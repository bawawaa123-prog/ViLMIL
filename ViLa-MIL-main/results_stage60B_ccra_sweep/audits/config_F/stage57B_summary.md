...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.2575，median=0.2121，ratio>0.5 占比=0.1340。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0060，csg_only acc=0.8247422680412371。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage60B_ccra_sweep/audits/config_F \
  --split test \
  --fold 0 \
  --run_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage60B_ccra_sweep/rce_v2_rcD_ccra_F_fold0_s1
```
