...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.3400，median=0.3311，ratio>0.5 占比=0.1340。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0028，csg_only acc=0.711340206185567。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage60C_ccra_configD_5fold/audits/fold_0 \
  --split test \
  --fold 0 \
