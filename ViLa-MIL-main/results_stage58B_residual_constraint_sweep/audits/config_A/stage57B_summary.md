...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.4858，median=0.4223，ratio>0.5 占比=0.3299。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0010，csg_only acc=0.2422680412371134。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58B_residual_constraint_sweep/audits/config_A \
  --split test \
  --fold 0 \
  --run_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58B_residual_constraint_sweep/rce_v2_rc_config_A_fold0_s1
```
