...
- visual residual 是否明显主导 final logits：`no_clear_dominance`。 依据：mean visual ratio=0.4441，median=0.3828，ratio>0.5 占比=0.2784。
- CSG 是否有可观察贡献：`yes`。 依据：mean csg ratio=0.0013，csg_only acc=0.5515463917525774。
- 下一步建议：`dynamic CSG`。
...
python scripts/analysis/build_stage57B_logit_contribution_audit.py \
  --output_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58B_residual_constraint_sweep/audits/config_C \
  --split test \
  --fold 0 \
  --run_dir /home/ljh/ViLMIL/ViLa-MIL-main/results_stage58B_residual_constraint_sweep/rce_v2_rc_config_C_fold0_s1
```
