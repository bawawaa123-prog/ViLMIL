...
PYTHONPATH="${ROOT_DIR}" \
  "${PYTHON_BIN}" "${ROOT_DIR}/scripts/aggregate_stage3_peps_extended_results.py"

echo "[Analyze] Fold-level deltas"
PYTHONPATH="${ROOT_DIR}" \
  "${PYTHON_BIN}" "${ROOT_DIR}/scripts/analyze_stage3_peps_fold_deltas.py"

echo "[Analyze] Prompt usage diversity"
PYTHONPATH="${ROOT_DIR}" \
  "${PYTHON_BIN}" "${ROOT_DIR}/scripts/analyze_stage3_peps_prompt_usage.py"

echo "[Report] Build markdown report"
PYTHONPATH="${ROOT_DIR}" \
  "${PYTHON_BIN}" "${ROOT_DIR}/scripts/build_stage3_peps_extended_report.py"
