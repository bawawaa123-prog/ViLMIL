...
OUTPUT_DIR = ROOT / "results_stage59A_dynamic_csg_all_off"
...
SMOKE_SCRIPT = ROOT / "scripts" / "experiments" / "run_stage59A_dynamic_csg_smoke.sh"
...
NEW_ARGS = [
    ...
    "rce_dynamic_csg_detach_evidence",
...
def build_dataset(settings: dict[str, object]) -> Generic_MIL_Dataset:
    ...
    if task == "task_adenocarcinoma":
        ...
        raise ValueError(f"Unsupported task for Step59A audit: {task}")
...
def compare_outputs(lhs: dict[str, object], rhs: dict[str, object], atol: float = 1e-7) -> dict[str, object]:
    ...
    return {
        ...
        "pass": bool(prob_diff <= atol and loss_diff <= atol and hat_equal),
        "atol": atol,
    }
...
def build_run_commands_text() -> str:
    ...
    return "\n".join(
        [
            f"cd {display_root}",
            "RUN_TRAIN=1 bash scripts/experiments/run_stage59A_dynamic_csg_smoke.sh",
            "",
            "# Refresh Step59A audit",
            f"PYTHONPATH={display_root} HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 {PYTHON_BIN} scripts/analysis/build_stage59A_dynamic_csg_audit.py",
        ]
    )
...
def build_summary_md(
    ...
    dynamic_params = ", ".join(NEW_ARGS)
    smoke_breakdown = smoke_audit.get("dynamic_breakdown", {}) or {}
    lines = [
        ...
        f"3. 新增了哪些 Dynamic CSG 参数：{dynamic_params}。",
        "4. Dynamic CSG 的计算方式是什么："
        " 先用 low/high concept evidence 构造 sample-level outer product，"
        "再按设定 norm 做稳定归一化，得到 delta_adj(sample)，"
        "最后用 dynamic_adj = static_adj + alpha * delta_adj 参与 cross-scale logits 计算。",
        f"5. alpha 初始化是多少：{smoke_audit.get('alpha_init', '0.0')}。",
        "6. dynamic-off 等价检查是否通过："
        f" {'是' if all_off_audit.get('dynamic_off_equivalence', {}).get('pass') else '否'}。",
        "7. full all-off 等价检查是否通过："
        f" {'是' if all_off_audit.get('full_all_off_equivalence', {}).get('pass') else '否'}。",
        "8. 开启 Dynamic CSG 后 smoke test 是否通过："
        f" {'是' if smoke_audit.get('smoke_pass') else '否'}。",
        "9. Step58C config D 的 residual constraint 机制是否仍然保留："
        f" {'是' if smoke_audit.get('residual_constraint_still_enabled') else '否'}。",
        "10. 下一步是否建议进入 Step59B Dynamic CSG 参数小扫："
        f" {'是' if smoke_audit.get('smoke_pass') and all_off_audit.get('dynamic_off_equivalence', {}).get('pass') and all_off_audit.get('full_all_off_equivalence', {}).get('pass') else '否'}。",
        "",
        "## Audit Notes",
        "",
        f"- forward return format preserved: `{config_audit.get('forward_return_format')}`",
        f"- dynamic default enabled flag: `{config_audit.get('dynamic_default_enabled')}`",
        f"- dynamic breakdown keys present: `{smoke_audit.get('dynamic_breakdown_keys_present')}`",
        f"- dynamic_csg_enabled in smoke: `{smoke_breakdown.get('dynamic_csg_enabled')}`",
        f"- dynamic_delta_mean/std in smoke: `{smoke_breakdown.get('dynamic_delta_mean')}` / `{smoke_breakdown.get('dynamic_delta_std')}`",
        f"- csg_logits_delta_abs_mean in smoke: `{smoke_breakdown.get('csg_logits_delta_abs_mean')}`",
    ...
    if smoke_audit.get("smoke_pass") and all_off_audit.get("dynamic_off_equivalence", {}).get("pass") and all_off_audit.get("full_all_off_equivalence", {}).get("pass"):
        lines.append("- 结论：可以进入 Step59B，对 Dynamic CSG 做参数小扫。")
    else:
        lines.append("- 结论：先修复 Step59A 审计未通过项，再考虑进入 Step59B。")
    return "\n".join(lines) + "\n"


def main() -> int:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    (OUTPUT_DIR / "stage59A_run_commands.txt").write_text(
    ...
    config_audit = {
        ...
        "model_has_dynamic_attrs": file_contains_tokens(MODEL_PATH, MODEL_ATTRS),
    ...
    dynamic_off_base_model, _ = instantiate_model(step58c_settings, STEP58C_CHECKPOINT, strict=True)
    dynamic_off_override_model, _ = instantiate_model(
        step58c_settings,
        STEP58C_CHECKPOINT,
        overrides={
    ...
    dynamic_off_base_output = run_model(dynamic_off_base_model, sample)
    dynamic_off_override_output = run_model(dynamic_off_override_model, sample)
    dynamic_off_equivalence = compare_outputs(dynamic_off_base_output, dynamic_off_override_output)
    ...
    dynamic_on_model, dynamic_on_load = instantiate_model(
        step58c_settings,
        STEP58C_CHECKPOINT,
        overrides={
    ...
    dynamic_on_output = run_model(dynamic_on_model, sample)
    dynamic_breakdown = dynamic_on_output.get("dynamic_csg_breakdown") or {}
    loss_breakdown = dynamic_on_output.get("loss_breakdown") or {}
    config_audit["forward_return_format"] = {
        "type": "tuple",
        "length": 3,
        "y_prob_shape": list(dynamic_on_output["y_prob"].shape),
        "y_hat_shape": list(dynamic_on_output["y_hat"].shape),
        "loss_is_scalar": bool(dynamic_on_output["loss"].numel() == 1),
    }
    config_audit["dynamic_default_enabled"] = False

    all_off_audit = {
        "dynamic_off_equivalence": dynamic_off_equivalence,
        "full_all_off_equivalence": full_all_off_equivalence,
    }
    smoke_audit = {
        "smoke_pass": bool(
            dynamic_breakdown.get("dynamic_csg_enabled") is True
            and all(dynamic_breakdown.get(key) is not None for key in BREAKDOWN_KEYS[:8])
            and dynamic_on_output.get("logit_breakdown") is not None
        ),
        "alpha_init": 0.0,
        "dynamic_breakdown": {
            key: round_or_none(dynamic_breakdown.get(key))
            if isinstance(dynamic_breakdown.get(key), (float, int, torch.Tensor))
            else dynamic_breakdown.get(key)
            for key in BREAKDOWN_KEYS + ["dynamic_csg_scale", "dynamic_csg_norm", "dynamic_csg_detach_evidence", "dynamic_csg_clip"]
        },
        "dynamic_breakdown_keys_present": {key: key in dynamic_breakdown for key in BREAKDOWN_KEYS},
        "load_state_dict": dynamic_on_load,
    ...
    summary_md = build_summary_md(config_audit, all_off_audit, smoke_audit)
    (OUTPUT_DIR / "stage59A_config_audit.json").write_text(
        json.dumps(config_audit, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    (OUTPUT_DIR / "stage59A_all_off_equivalence_audit.json").write_text(
        json.dumps(all_off_audit, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    (OUTPUT_DIR / "stage59A_dynamic_csg_smoke.json").write_text(
        json.dumps(smoke_audit, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    (OUTPUT_DIR / "stage59A_summary.md").write_text(summary_md, encoding="utf-8")
    return 0
