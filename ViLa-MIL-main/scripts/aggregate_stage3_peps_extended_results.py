...
def _collect_one(experiment: str, train_dir: Path):
    row = {
    ...
    result_path = train_dir / "result.csv"
    fold_path = train_dir / "fold_summary.csv"
...
def main():
    ...
    experiments = [
        ...
        ("Concept-12 PEPS topk=3 tau=0.07", root / "trained_models" / "adeno_concept12_peps_topk3_tau0.07_s1"),
        ("Concept-12 PEPS topk=3 tau=0.15", root / "trained_models" / "adeno_concept12_peps_topk3_tau0.15_s1"),
        ("Concept-12 PEPS topk=5 tau=0.07", root / "trained_models" / "adeno_concept12_peps_topk5_tau0.07_s1"),
        ("Concept-12 PEPS topk=5 tau=0.15", root / "trained_models" / "adeno_concept12_peps_topk5_tau0.15_s1"),
    ]

    summary_rows = []
    fold_frames = []
    for experiment_name, train_dir in experiments:
        row, fold_df = _collect_one(experiment_name, train_dir)
    ...
    summary_csv = output_dir / "peps_extended_summary.csv"
    ...
    print("\nExtended PEPS summary:")
    print(summary_df[display_cols].to_string(index=False))
    print(f"\nSaved extended summary table to: {summary_csv}")
    print(f"Saved extended fold-level table to: {folds_csv}")
