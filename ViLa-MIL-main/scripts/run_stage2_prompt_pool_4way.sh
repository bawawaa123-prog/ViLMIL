...
CONCEPT6_PATH="${CONCEPT6_PATH:-${ROOT_DIR}/dataset_csv/private_lung_concept_prompt_pool_stage2.json}"
CONCEPT10_PATH="${CONCEPT10_PATH:-${ROOT_DIR}/dataset_csv/private_lung_concept_prompt_pool_stage2_core10.json}"
...
echo "[1/4] BiomedCLIP static prompt baseline"
echo "Skip rerun. Reuse existing result: ${STATIC_BASELINE_DIR}"

echo "[2/4] Concept-6 embedding_mean"
echo "Skip rerun. Reuse existing result: ${CONCEPT6_RESULT_DIR}"

echo "[3/4] Concept-10 embedding_mean"
run_train "adeno_concept10_embedding_mean" \
  --use_concept_prompt_pool \
  --concept_prompt_path "${CONCEPT10_PATH}" \
  --prompt_ensemble_mode embedding_mean

echo "[4/4] Concept-10 logit_mean"
run_train "adeno_concept10_logit_mean" \
  --use_concept_prompt_pool \
  --concept_prompt_path "${CONCEPT10_PATH}" \
  --prompt_ensemble_mode logit_mean
