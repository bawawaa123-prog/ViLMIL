...
RESULTS_DIR="${RESULTS_DIR:-results_stage60B_ccra_sweep}"
...
RUN_COMMANDS_FILE="${OUTPUT_DIR}/stage60B_run_commands.txt"
...
run_config() {
  ...
  local -a cmd=(
    ...
    --rce_ccra_alpha_init "${alpha_init}"
  ...
  if [[ "${use_detach_prompt}" == "1" ]]; then
    cmd+=(--rce_ccra_detach_prompt)
  fi
...

{
  ...
  echo "RUN_TRAIN=1 bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo "RUN_TRAIN=1 CONFIGS=extended bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo
  echo "# Single config examples"
  echo "RUN_TRAIN=1 CONFIGS=A bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo "RUN_TRAIN=1 CONFIGS=B bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo "RUN_TRAIN=1 CONFIGS=C bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo "RUN_TRAIN=1 CONFIGS=D bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo "RUN_TRAIN=1 CONFIGS=E bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo "RUN_TRAIN=1 CONFIGS=F bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo "RUN_TRAIN=1 CONFIGS=G bash scripts/experiments/run_stage60B_ccra_sweep.sh"
  echo
  echo "# Refresh Step60B summary"
  echo "PYTHONPATH=${DISPLAY_ROOT} HF_HUB_OFFLINE=${HF_HUB_OFFLINE_FLAG} TRANSFORMERS_OFFLINE=${TRANSFORMERS_OFFLINE_FLAG} ${PYTHON_BIN} scripts/analysis/build_stage60B_ccra_sweep_summary.py"
} > "${RUN_COMMANDS_FILE}"
