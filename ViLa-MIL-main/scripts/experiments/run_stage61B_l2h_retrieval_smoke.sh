...
RESULTS_DIR="${RESULTS_DIR:-results_stage61B_l2h_retrieval_all_off}"
RUN_TRAIN="${RUN_TRAIN:-0}"
SEED="${SEED:-1}"
MAX_EPOCHS="${MAX_EPOCHS:-1}"
EXP_CODE="${EXP_CODE:-rce_v2_rcD_l2h_smoke}"
...
RUN_COMMANDS_FILE="${OUTPUT_DIR}/stage61B_run_commands.txt"
...

{
  ...
  echo "RUN_TRAIN=1 bash scripts/experiments/run_stage61B_l2h_retrieval_smoke.sh"
  echo
  echo "# Refresh Step61B audit"
  echo "PYTHONPATH=${DISPLAY_ROOT} HF_HUB_OFFLINE=${HF_HUB_OFFLINE_FLAG} TRANSFORMERS_OFFLINE=${TRANSFORMERS_OFFLINE_FLAG} ${PYTHON_BIN} scripts/analysis/build_stage61B_l2h_retrieval_audit.py"
} > "${RUN_COMMANDS_FILE}"

echo "[Run] Stage61B L2H Retrieval smoke"
...
echo "[Run] SEED=${SEED}"
echo "[Run] L2H_MODE=low_topk_coord_window"
echo "[Run] L2H_LOW_TOPK=8"
echo "[Run] L2H_HIGH_MAX_PER_LOW=16"
echo "[Run] L2H_SCALE_RATIO=1.0"
echo "[Run] L2H_PATCH_FOOTPRINT_RATIO=4.0"
echo "[Run] L2H_ALPHA_INIT=0.0"
echo "[Run] RUN_COMMANDS_FILE=${RUN_COMMANDS_FILE}"
print_command env "${env_vars[@]}" "${cmd[@]}"

if [[ "${RUN_TRAIN}" == "1" ]]; then
  env "${env_vars[@]}" "${cmd[@]}"
  echo "[Done] Stage61B smoke finished"
