...
RESULTS_DIR="${RESULTS_DIR:-results_stage34}"
SEED="${SEED:-1}"
MAX_EPOCHS="${MAX_EPOCHS:-1}"
VARIANT="${VARIANT:-gate001}"
...
CSG_INIT=0.1
SUPPORTED_VARIANTS=(gate0 gate001 gate005 all)
RUN_ALL_VARIANTS=(gate0 gate001 gate005)
...
run_variant() {
  local variant="$1"
  local exp_code="stage34_visual_gate_smoke_${variant}"
  local gate_init=""
  ...
  echo "[Run] Stage34 Visual Evidence-Level Gate smoke"
  echo "[Run] VARIANT=${variant}"
  echo "[Run] GATE_INIT=${gate_init}"
  echo "[Run] PROTOTYPE_NUMBER=${PROTOTYPE_NUMBER}"
  echo "[Run] VISUAL_RESIDUAL_INIT=${VISUAL_RESIDUAL_INIT}"
