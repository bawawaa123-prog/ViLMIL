...
echo "[1/4] Smoke train: SAF-PEPS learned_gate, fold0, 1 epoch"
PYTHONPATH="${ROOT_DIR}" \
...
echo "[2/4] Full 5-fold train: SAF-PEPS learned_gate"
PYTHONPATH="${ROOT_DIR}" \
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 \
"${PYTHON_BIN}" "${ROOT_DIR}/main.py" \
  --task task_adenocarcinoma \
  ...
  --split_dir "${SPLIT_DIR}" \
  --results_dir "${RESULTS_DIR}" \
  --exp_code adeno_concept12_saf_peps_topk5_tau0.07 \
  --scale_fusion_mode learned_gate \
  --scale_gate_hidden_dim "${SCALE_GATE_HIDDEN_DIM}" \
  --scale_gate_dropout "${SCALE_GATE_DROPOUT}"
...
"${PYTHON_BIN}" "${ROOT_DIR}/eval.py" \
  ...
  --data_folder_l features_biomedclip_20x \
  --results_dir "${RESULTS_DIR}" \
  --models_exp_code adeno_concept12_saf_peps_topk5_tau0.07_s1 \
  --save_exp_code adeno_concept12_saf_peps_topk5_tau0.07 \
  --split test \
  --k "${K_FOLDS}" \
  --k_start 0 \
  --k_end 5 \
...
echo "[4/4] Build SAF-PEPS comparison summary"
PYTHONPATH="${ROOT_DIR}" \
  "${PYTHON_BIN}" "${ROOT_DIR}/scripts/analysis/build_stage7_saf_peps_analysis.py"

echo "[Done] Outputs:"
echo "  Train dir: ${ROOT_DIR}/trained_models/adeno_concept12_saf_peps_topk5_tau0.07_s1"
echo "  Eval dir : ${ROOT_DIR}/eval_results/EVAL_adeno_concept12_saf_peps_topk5_tau0.07"
echo "  Summary  : ${ROOT_DIR}/trained_models/stage7_saf_peps_comparison/saf_peps_comparison_summary.csv"
