...
RESULTS_DIR="${RESULTS_DIR:-results_stage59A_dynamic_csg_all_off}"
RUN_TRAIN="${RUN_TRAIN:-0}"
SEED="${SEED:-1}"
MAX_EPOCHS="${MAX_EPOCHS:-1}"
EXP_CODE="${EXP_CODE:-rce_v2_rcD_dynamic_csg_smoke}"
...
RUN_COMMANDS_FILE="${OUTPUT_DIR}/stage59A_run_commands.txt"
...

{
  ...
  echo "RUN_TRAIN=1 bash scripts/experiments/run_stage59A_dynamic_csg_smoke.sh"
  echo
  echo "# Refresh Step59A audit"
  echo "PYTHONPATH=${DISPLAY_ROOT} HF_HUB_OFFLINE=${HF_HUB_OFFLINE_FLAG} TRANSFORMERS_OFFLINE=${TRANSFORMERS_OFFLINE_FLAG} ${PYTHON_BIN} scripts/analysis/build_stage59A_dynamic_csg_audit.py"
} > "${RUN_COMMANDS_FILE}"

echo "[Run] Stage59A Dynamic CSG smoke"
...
if [[ "${RUN_TRAIN}" == "1" ]]; then
  env "${env_vars[@]}" "${cmd[@]}"
  echo "[Done] Stage59A smoke finished"
