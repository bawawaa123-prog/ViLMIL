...
RESULTS_DIR="${RESULTS_DIR:-results_stage36}"
SEED="${SEED:-1}"
MAX_EPOCHS="${MAX_EPOCHS:-1}"
VARIANT="${VARIANT:-lh_l001_m0}"
K_START="${K_START:-0}"
K_END="${K_END:-0}"
...
CSG_INIT=0.1
SUPPORTED_VARIANTS=(lh_l001_m0 lh_l005_m0 lh_l001_m005 lh_l005_m005 all)
RUN_ALL_VARIANTS=(lh_l001_m0 lh_l005_m0 lh_l001_m005 lh_l005_m005)
...
run_variant() {
  local variant="$1"
  local exp_code="stage36_lh_consistency_smoke_${variant}"
  local lh_lambda=""
  local lh_margin=""
  ...
  variant_args+=(
    --rce_lh_consistency_lambda "${lh_lambda}"
    --rce_lh_consistency_margin "${lh_margin}"
  )
  ...
  echo "[Run] Stage36 Low-High Consistency smoke"
  echo "[Run] VARIANT=${variant}"
  echo "[Run] LH_LAMBDA=${lh_lambda}"
  echo "[Run] LH_MARGIN=${lh_margin}"
...
case "${VARIANT}" in
  lh_l001_m0|lh_l005_m0|lh_l001_m005|lh_l005_m005)
    run_variant "${VARIANT}"
    ;;
  all)
