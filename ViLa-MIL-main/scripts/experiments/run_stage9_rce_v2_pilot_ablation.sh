...
run_variant() {
  ...
  case "${variant}" in
    calib_only)
      exp_code="rce_mil_v2_calib_fold0_e20"
      extra_args=(
      ...
      ;;
    prior_only)
      exp_code="rce_mil_v2_prior_fold0_e20"
      extra_args=(
      ...
      ;;
    prior_calib)
      exp_code="rce_mil_v2_prior_calib_fold0_e20"
      extra_args=(
        --rce_use_logit_calibration
        --rce_use_concept_prior
        --rce_logit_scale_init 10.0
    ...
    *)
      ...
      echo "[Error] Supported VARIANT values: calib_only, prior_only, prior_calib, all" >&2
      exit 1
      ;;
  esac

  echo "[Run] RCE-MIL v2 variant: ${variant}"
...
case "${VARIANT}" in
  calib_only|prior_only|prior_calib)
    run_variant "${VARIANT}"
    ;;
  all)
    run_variant calib_only
